...
import type { CustomerAnalysis } from "@/types/reviews";
import { getCustomerAnalysis } from "@/lib/reviewsApi";
...
const CustomerAnalysis: React.FC<Props> = ({ airlineSlug }) => {
  const { data, error, isLoading, mutate } = useSWR<CustomerAnalysis>(
    ...
    () => getCustomerAnalysis(airlineSlug)
  )

  const reviewsByCountry = data?.reviewsByCountry ?? [];
  const verifiedAndUnverifiedReviews = data?.verifiedAndUnverifiedReviews ?? { verified: 0, unverified: 0 };
  const aircraftSeatTypePercentage = data?.aircraftSeatTypePercentage ?? [];
  const travellerTypePercentage = data?.travellerTypePercentage ?? [];
