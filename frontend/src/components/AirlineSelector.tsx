...
import { getAllAirlines } from "@/lib/reviewsApi";
...
export default function AirlineSelectorClient({ airlineSlug }: Props) {
  ...
  // Map API -> local shape with slug
  const airlines = useMemo(
    () =>
      (data ?? []).map((a) => ({
        airline: a.airline,
        slug: slugify(a.airline),
      })),
    [data]
  );

  const sorted = useMemo(
    () => [...airlines].sort((a, b) => a.airline.localeCompare(b.airline)),
    [airlines]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter(
      (a) =>
        a.airline.toLowerCase().includes(s) || a.slug.toLowerCase().includes(s)
    );
  }, [q, sorted]);
  ...
  useEffect(() => setOpen(false), [airlineSlug]);
  ...
  return (
    <div className="card bg-white dark:bg-gray-800 p-6 rounded-2xl mb-6 shadow-sm">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {isLoading ? "Loading…" : `${airlines.length} available`}
          </p>
        </div>
        <button
          onClick={copyLink}
      ...
      {isLoading ? (
        ...
        <>
          {/* Top quick-select pills (alphabetical first 6) */}
          <div className="flex flex-wrap gap-2 mb-4">
            {top.map((a) => {
              const selected = airlineSlug === a.slug;
              return (
                <button
                  key={a.slug}
                    ...
                    return (
                      <li key={a.slug}>
                        ...
                        >
                          ...
                          <span className="text-xs opacity-60">{a.slug}</span>
                        </button>
                      </li>
                    );
                  })
